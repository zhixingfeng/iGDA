                                   ...
                                   vector<bool> &visited, int &n_split, set<int64_t> &end_vertex_id)
{
    ...
    if (it->second.size() != 1){
        ...
        auto it_in_next =  gp.adj_mat_in.find(it->second[0].id);
        if (it_in_next == gp.adj_mat_in.end())
            throw runtime_error("get_unambigious_paths_ua_core(): vertex is not found in gp");
        
        // continue to search daugter vertices if next vertex has in degree <= 1
        if (it_in_next->second.size() > 1){
            path_all.insert(path);
            for (auto v_next : it->second){
                if (!visited[v_next.id])
                    end_vertex_id.insert(v_next.id);
            ...
            for (auto v_next : it->second){
                if (!visited[v_next.id])
                    get_unambigious_paths_ns_core(gp, v_next.id, path, path_all, visited, n_split, end_vertex_id);
            }
        }
    }
...
set<vector<int64_t> > get_unambigious_paths_ns(const IGDA_Graph &gp)
{
    ...
    while(v_active.size() > 0){
        ...
        if (v_visisted.find(cur_v) == v_visisted.end()){
            ...
            int n_split = 0;
            set<int64_t> end_vertex_id;
            get_unambigious_paths_ns_core(gp, cur_v, path, ab_paths,
                                          v_visited_core, n_split, end_vertex_id);
